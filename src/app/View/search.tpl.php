...
<?php
if ($view->getSession()->getFlashBag()->has('notice')){
    $error = $view->getSession()->getFlashBag()->get('notice');
    echo "<h3 style=\"margin-left: 250px;margin-top: 80px;color: #64BB81;\">".$error[0]."</h3>";
    die();
}
if(empty($searchList))
    echo "<h3 style=\"margin-left: 250px;margin-top: 80px;color: #64BB81;\">Also, There is no related articles...</h3>";
?>
<?php foreach($searchList as $article){ ?>
                <article class='excerpt excerpt-1'>
                    ...
                    <div class='excerpt-right'>
                        <header>
                            <h2><a href="<?php echo $view->Route('articleshow/'.$article['id']); ?>" title="<?php echo $article['title']?>" rel='bookmark'><?php echo str_replace($keyword,"<span style=\"color:#1FD55D\">$keyword</span>", $article['title']);?></a></h2>
                        </header>
                        <p class='meta'>
